...
if(isset($_POST['id'])) {
	...
	if($db->RowCount > 0) {
		foreach($rows as $row) {
			if($row['icon']=='fa-car') $title = '[HỒ SƠ]: ' . stripslashes($row['name']) . ' (' . $date->vnDateTime($row['created_time']) . ')';
			...
			else $title = '[ĐẶT HÀNG ONLINE]: ' . stripslashes($row['name']) . ' (' . $date->vnDateTime($row['created_time']) . ')';
