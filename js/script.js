...
function sendMail2(type, id) {
	...
	jQuery.ajax({
		...
		dataType: "html",
		beforeSend: function(){
			$('#_send_order').prop('disabled', true);
		},
		success: function(data){
			$('#_send_order').prop('disabled', false);
			closeLoader();
			jQueryquery_unt('#'+id);
...
function check_order(){
	...
	var select = document.forms['frm_order'].getElementsByTagName('select');
	var err_field = jQuery('#txtEnterField').val();
	var err_email = jQuery('#txtEnterMail').val();
	var err_tell = jQuery('#txtEnterTell').val();

	var inputs = new Array();
	for(var i=0; i<(input.length+txtarea.length+select.length); i++){
		if(i<input.length) inputs[i]=input[i];
		else if(i < input.length + txtarea.length) inputs[i]=txtarea[i-input.length];
		else inputs[i]=select[i-input.length -txtarea.length];
	}
	var run_onchange = false;
		...
		for(var i=0; i<inputs.length; i++){
			...
			if( value == ''){
				if(id == 'txtName') {
					...
					span.innerHTML = 'Tỉnh thành chưa được chọn';
				}
				else if(id == 'districtTxt') {
					span.innerHTML = 'Quận huyện chưa được chọn';
				}
...
// function owl_sanphamnoibat(){
// 	var owl = jQuery('.sanphamnoibat');
// 	btt: true,
// 	owl.owlCarousel({
// 	    loop:true,
// 	    margin:30,
...
// 	});
// }
function owl_banner_sp(){
	var owl = jQuery('.banner_sp');
	btt: true,
	owl.owlCarousel({
	    loop:true,
	    margin:0,
